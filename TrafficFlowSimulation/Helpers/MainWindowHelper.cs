...
public class MainWindowHelper
{
	...
	public void InitializeTableLayoutPanelComponent()
	{
		...
		var basicParametersTableLayoutPanelComponent = new TableLayoutPanelComponent(
			...
			_controls.Find(ControlName.MainWindowControlName.BasicParametersTableLayoutPanel, true).Single() as
				TableLayoutPanel,
			_bindingSources,
			_errorProvider);
		...
		var additionalParametersTableLayoutPanelComponent = new TableLayoutPanelComponent(
			...
			_controls.Find(ControlName.MainWindowControlName.AdditionalParametersTableLayoutPanel, true).Single() as
				TableLayoutPanel,
			_bindingSources,
			_errorProvider);
		...
		var initialConditionsTableLayoutPanelComponent = new TableLayoutPanelComponent(
			...
			_controls.Find(ControlName.MainWindowControlName.InitialConditionsTableLayoutPanel, true).Single() as
				TableLayoutPanel,
			_bindingSources,
			_errorProvider);
	...
	{
		var currentDrivingMode = ModesHelper.GetCurrentDrivingMode();
		var settingsTableLayoutPanel =
			_controls.Find(ControlName.MainWindowControlName.SettingsTableLayoutPanel, true).Single() as
				TableLayoutPanel;
	...
	{
		var controlMenuStrip =
			_controls.Find(ControlName.MainWindowControlName.ControlMenuStrip, true).Single() as ToolStrip;
		var drivingModeStripDropDownButton =
			controlMenuStrip?.Items.Find(ControlName.MainWindowControlName.DrivingModeStripDropDownButton, false)
				.Single() as ToolStripDropDownButton;

		if (drivingModeStripDropDownButton != null)
		{
			if (_controls.Find(ControlName.CommonControlName.ParametersPanelName, true).Single() is not Panel
			    parametersPanel)
	...
	private void InitializeLanguageComponent()
	{
		var controlMenuStrip =
			_controls.Find(ControlName.MainWindowControlName.ControlMenuStrip, true).Single() as ToolStrip;
		var languagesSwitcherButton =
			controlMenuStrip?.Items.Find(ControlName.MainWindowControlName.LanguagesSwitcherButton, false).Single() as ToolStripDropDownButton;
	...
	public void ResizeChart(Chart chart)
	{
		...
		{
			ChartAreaRendersHelper.CreateCustomLabels(chartArea.AxisX,
				Math.Abs(chartArea.AxisX.ValueToPixelPosition(0) - chartArea.AxisX.ValueToPixelPosition(1)));
			ChartAreaRendersHelper.CreateCustomLabels(chartArea.AxisY,
				Math.Abs(chartArea.AxisY.ValueToPixelPosition(0) - chartArea.AxisY.ValueToPixelPosition(1)));
		}
	}

	public void CreateRandomFlow()
	{
		var comboBoxes = FindComboBoxes(_controls).ToList();
		foreach (var source in _bindingSources)
		{
			if (source.Value.DataSource is IRandomParametersModel randomParametersModel)
			{
				source.Value.DataSource = randomParametersModel.GetRandomValues();

				var properties = source.Value.DataSource.GetType().GetProperties();
				foreach (var property in properties.Where(x => x.PropertyType == typeof(EnumItem)))
				{
					var comboBox = comboBoxes.FirstOrDefault(x => x.Name.Contains(property.Name));

					if (comboBox != null)
					{
						var enumProperty = (EnumItem) property.GetValue(source.Value.DataSource);
						foreach (var item in comboBox.Items)
						{
							var enumItem = (EnumItem) item;
							if (enumItem.Value.Equals(enumProperty.Value))
							{
	...
	private IEnumerable<ComboBox> FindComboBoxes(Control.ControlCollection controls)
	{
		return controls.OfType<ComboBox>().Concat(controls.Cast<Control>().SelectMany(c => FindComboBoxes(c.Controls)));
	}
}
